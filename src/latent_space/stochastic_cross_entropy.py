...
class StochasticSkipCrossEntropy(torch.nn.Module):
    def __init__(
        ...
        if not (0.0 <= p_on_correct <= 1.0):
            msg = "p_on_correct must be in [0, 1]."
        ...
        self.register_buffer(
            "weight",
            weight if weight is None else torch.as_tensor(weight, dtype=torch.float),
        )
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self._ext_generator is not None:
                r = torch.rand(
                    correct.sum().item(),
                    generator=self._ext_generator,
                    device=ce_logits.device,
                )
                include_correct = torch.zeros_like(correct, dtype=torch.bool)
                include_correct[correct] = r < self.p_on_correct
            else:
                include_correct = torch.zeros_like(correct, dtype=torch.bool)
                include_correct[correct] = (
                    torch.rand(correct.sum().item(), device=ce_logits.device) < self.p_on_correct
                )
