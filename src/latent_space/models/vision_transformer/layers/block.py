...
class SelfAttentionBlock(nn.Module):
    def __init__(
        ...
        self.use_skip = not self.use_mhc
        # -----------
        ...
        if self.use_mhc:
            ...
            self.hc_attn = init_mhc(
                dim=dim,
                branch=AttnBranch(self.norm1, self.attn, self.ls1),
                layer_index=layer_idx * 2,
                **mhc_kwargs,
            )
            self.hc_mlp = init_mhc(
                dim=dim,
                branch=MlpBranch(self.norm2, self.mlp, self.ls2),
    ...
    def _forward(self, x: Tensor, rope=None) -> Tensor:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            # )
            # Added flag to disable residual connection
            base_1 = x if self.use_skip else torch.zeros_like(x)
            x_attn = torch.index_add(
                base_1,
                dim=0,
                source=self.ls1(residual_1),
            ...
            # )
            # Added flag to disable residual connection
            base_2 = x_attn if self.use_skip else torch.zeros_like(x_attn)
            x_ffn = torch.index_add(
                base_2,
                dim=0,
                source=self.ls2(residual_2),
        ...
        else:  # Added flag to disable residual connection
            attn_out = self.ls1(self.attn(self.norm1(x), rope=rope))
            x_attn = (x + attn_out) if self.use_skip else attn_out

            ffn_out = self.ls2(self.mlp(self.norm2(x_attn)))
            x_ffn = (x_attn + ffn_out) if self.use_skip else ffn_out
    ...
    def _forward_list(self, x_list: list[Tensor], rope_list=None) -> list[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            x_subset_1_list = [
                x[indices_1] for x, indices_1 in zip(x_list, indices_1_list, strict=False)
            ]
            ...
            x_attn_list = [
                torch.index_add(
                    # x,
                    # Added flag to disable residual connection
                    (x if self.use_skip else torch.zeros_like(x)),
                    dim=0,
                    source=self.ls1(residual_1),
                    index=indices_1,
                    alpha=residual_scale_factor,
            ...
            x_subset_2_list = [
                x[indices_2] for x, indices_2 in zip(x_attn_list, indices_2_list, strict=False)
            ]
            ...
            x_ffn = [
                torch.index_add(
                    # x_attn,
                    # Added flag to disable residual connection
                    (x_attn if self.use_skip else torch.zeros_like(x_attn)),
                    dim=0,
                    source=self.ls2(residual_2),
                    index=indices_2,
                    alpha=residual_scale_factor,
