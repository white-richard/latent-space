...
def _build_mhc_variant(config: ConfigT) -> ConfigT:
    variant = copy.deepcopy(config)

    if not getattr(variant.model, "model_name", "").endswith("_mhc"):
        variant.model.model_name = f"{variant.model.model_name}_mhc"

    if hasattr(variant.experiment, "output_dir"):
        variant.experiment.output_dir = Path(variant.experiment.output_dir) / "mhc"

    if hasattr(variant.experiment, "is_mhc"):
        variant.experiment.is_mhc = True

    if hasattr(variant.experiment, "experiment_name"):
        variant.experiment.experiment_name = f"{variant.experiment.experiment_name}_mhc"

    return variant


def _collect_variant_builders(
    config: ConfigT,
    variant_builders: Iterable[Callable[[ConfigT], ConfigT | None]] | None,
) -> list[Callable[[ConfigT], ConfigT | None]]:
    builders: list[Callable[[ConfigT], ConfigT | None]] = []
    if getattr(config.experiment, "run_mhc_variant", False):
        builders.append(_build_mhc_variant)

    experiment_variants = getattr(config.experiment, "variant_builders", None)
    if experiment_variants:
        builders.extend(experiment_variants)

    if variant_builders:
        builders.extend(list(variant_builders))

    return builders


def expand_w_mhc(
    config: ConfigT, variant_builders: Iterable[Callable[[ConfigT], ConfigT | None]] | None = None
) -> list[ConfigT]:
    """Return configs including optional variants based on ExperimentConfig or provided builders."""
    configs: list[ConfigT] = [copy.deepcopy(config)]
    seen_experiment_names: set[str] = {getattr(config.experiment, "experiment_name", "")}

    for build in _collect_variant_builders(config, variant_builders):
        variant = build(copy.deepcopy(config))
        if variant is None:
            continue

        experiment_name = getattr(variant.experiment, "experiment_name", None)
        if experiment_name is not None and experiment_name in seen_experiment_names:
            continue
