...
def write_config_report(output_dir: Path, config: Any, *, filename: str = "config.md") -> Path:
...
def build_variant_section(
    *,
    ...
    if report_path.exists():
        ...
        status_line = _extract_status_line(report_text)
        if status_line:
            lines.append(status_line)
        lines.append(f"**Full Report:** ![[{report_path.name}]]")

        results_section = _extract_section(report_text, "## 📊 Results & Plots")
        if results_section:
            lines.append("")
            lines.append("### Results & Plots")
            lines.extend(results_section)
        elif results_path.exists():
            lines.append("")
            lines.append("### Results Table")
            lines.append(results_path.read_text(encoding="utf-8").strip())
        else:
            lines.append("_No results logged yet._")
    else:
        lines.append("_No report available._")
    return lines
...
def write_experiment_report(
    ...
    *,
    ...
    for label, report_path, results_path in variants:
        lines.append("")
        lines.extend(
