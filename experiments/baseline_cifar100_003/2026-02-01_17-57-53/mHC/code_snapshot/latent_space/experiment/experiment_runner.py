...
def prepare_experiment_artifacts(
    ...
    archive_experiment_state(_resolve_output_dir(config), code_dirs=code_dirs)
...
def run_experiment_with_variants(
    ...
    *,
    ...
    """
    results: list[Any] = []
    summary_rows: list[tuple[str, Path, Path]] = []
    report_writer = ObsidianReportWriter(report_config or get_default_report_config())
    summary_title = _safe_getattr(_safe_getattr(base_config, "experiment"), "experiment_name")
    summary_title = summary_title or "experiment"
    summary_project = resolve_project_name(base_config, report_writer.config)
    ...
    for idx, config in enumerate(expand_with_variants(base_config, variant_builders)):
        ...
        log_metrics_table(results_path, result)
        report_path = report_writer.write_report(
            output_dir, config=config, result=result, status_error=error
        )
        ...
        summary_rows.append((label, report_path, results_path))
...
def aggregate_seeds_run_experiment_with_variants(
    ...
    *,
    ...
    for idx, config in enumerate(expanded_configs):
        ...
        output_dir = _resolve_output_dir(config)
        results_path = output_dir / "results.md"
        log_metrics_table(results_path, averaged)
        report_path = report_writer.write_report(
            output_dir, config=config, result=averaged, status_error=None
        )
