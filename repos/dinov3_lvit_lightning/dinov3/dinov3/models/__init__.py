...
def build_model(args, only_teacher=False, img_size=224, device=None):
    if args.arch == "lvit_small":
        # Custom encoder path (optional dependency).
        # NOTE: To use this, set `student.embed_dim` in config if it cannot be inferred.
        lvit_kwargs = {}
        if hasattr(args, "lvit_kwargs") and args.lvit_kwargs is not None:
            lvit_kwargs = dict(args.lvit_kwargs)
        teacher = lvits.__dict__[args.arch](
        ...
        student = lvits.__dict__[args.arch](
            patch_size=args.patch_size,
            n_storage_tokens=args.n_storage_tokens,
            device=device,
            embed_dim=getattr(args, "embed_dim", None),
