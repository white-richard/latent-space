...
if __name__ == "__main__":
    ...
    # === Set these paths ===
    model_checkpoint = '/home/richiewhite/.code/latent-space/repos/dinov3_lvit_lightning/output/checkpoints/model_epoch_epoch=00_step_step=255000_loss_total_loss=8.875000.ckpt'
    cfg_path = '/home/richiewhite/.code/latent-space/repos/dinov3_lvit_lightning/configs/ssl_lvit_small.yaml'
    # =======================

    # Reroute dinov3_lightning imports
    sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../..")))

    # Load and save the model weights to a temporary path
    from src.checkpointing.model_checkpoint import load_dinov3_checkpoint
    model = load_dinov3_checkpoint(checkpoint_path=model_checkpoint, cfg_path=cfg_path, save_path="tmp/dinov3_model_weights.pt")

    # Ensure ImageNet datasets have their extra files dumped
    from dinov3.data.datasets import ImageNet
    for split in ImageNet.Split:
        dataset = ImageNet(split=split, root="/home/richiewhite/.code/datasets/imagenet", extra="/home/richiewhite/.code/datasets/imagenet")
        dataset.dump_extra()
    
    main()

""" LViT k-NN evaluation command example:
PYTHONPATH=. python -m dinov3.eval.knn \
  model.config_file=/home/richiewhite/.code/latent-space/repos/dinov3_lvit_lightning/configs/ssl_lvit_small.yaml \
  model.pretrained_weights=tmp/dinov3_model_weights.pt \
  train.dataset=ImageNet:root=/home/richiewhite/.code/datasets/imagenet:extra=/home/richiewhite/.code/datasets/imagenet:split=VAL \
  eval.test_dataset=ImageNet:root=/home/richiewhite/.code/datasets/imagenet:extra=/home/richiewhite/.code/datasets/imagenet:split=VAL \
  train.skip_first_nn=true \
  train.batch_size=64 \
  eval.batch_size=64 \
