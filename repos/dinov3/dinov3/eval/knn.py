...
PYTHONPATH=. python -m dinov3.eval.knn \
  ...
  train.dataset=ImageNet:root=/path/to/imagenet:split=TRAIN \
  eval.test_dataset=ImageNet:root=/path/to/imagenet:split=VAL \
  train.batch_size=64 train.num_workers=4 eval.batch_size=64 \
  train.ks=[1,5,10] \
  save_results=true \
  --output-dir /tmp/knn_out
"""